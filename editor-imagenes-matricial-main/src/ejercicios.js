...
// ============================================
// Nombre del estudiante: Marco David Parera Flores
// Fecha: 17/11/2025
// Grupo: 1-A
...
function imagenAMatriz(rutaImagen) {
  ...
  // Necesitamos importar 'fs' (Node.js) y el paquete PNG (ej: pngjs) para que esto funcione.
  const fs = require('fs');
  const PNG = require('pngjs').PNG;
  
  // Lee el archivo en un buffer
  const buffer = fs.readFileSync(rutaImagen);
  // Parsea el buffer para obtener el objeto PNG
  ...
  for (let y = 0; y < png.height; y++) {
    ...
    for (let x = 0; x < png.width; x++) {
      ...
      // El formato es R, G, B, A, y cada píxel ocupa 4 bytes.
      // Índice = (ancho * fila + columna) * 4
      const idx = (png.width * y + x) * 4;

      // 5. Extraer los valores RGBA
      const pixel = {
        r: png.data[idx],     // Componente Red
        g: png.data[idx + 1], // Componente Green
        b: png.data[idx + 2], // Componente Blue
        a: png.data[idx + 3]  // Componente Alpha
      };

      // Agregar el objeto píxel a la fila actual
      fila.push(pixel);
    }
    
    // Agregar la fila completa a la matriz principal
    matriz.push(fila);
  }
